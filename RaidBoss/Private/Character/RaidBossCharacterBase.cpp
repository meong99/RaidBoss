...
ARaidBossCharacterBase::ARaidBossCharacterBase()
{
    ...
    static ConstructorHelpers::FObjectFinder<UDataTable> ItemDataTableObject(
        TEXT("/Script/Engine.DataTable'/Game/Data/DataTables/DT_ItemData.DT_ItemData'"));
...
int32 ARaidBossCharacterBase::DecreaseOrRemoveInventoryData(FGameplayTag InAbilityTriggerTag,
...
void ARaidBossCharacterBase::ApplyCharacterDefaultSpecEffectToSelf()
{
    if (AbilitySystemComponent)
    {
        ...
        FGameplayEffectSpecHandle OutGoingSpec = AbilitySystemComponent->MakeOutgoingSpec(
            CharacterStatusEffect, 1, ContextHandle);

        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_MaxHealth,
                                                   CharacterDefaultSpec.MaxHealth);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_MaxMana,
                                                   CharacterDefaultSpec.MaxMana);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_AttackPower,
                                                   CharacterDefaultSpec.AttackPower);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_AttackRange,
                                                   CharacterDefaultSpec.AttackRange);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_DefensePower,
                                                   CharacterDefaultSpec.DefencePower);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_MoveSpeed,
                                                   CharacterDefaultSpec.MoveSpeed);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Init_Character_AttackSpeed,
                                                   CharacterDefaultSpec.AttackSpeed);
...
void ARaidBossCharacterBase::GiveDefaultAbilities() const
{
    ...
    for (const auto Ability : DefaultAbilities)
    {
        if (Ability)
        {
            ...
            AbilitySystemComponent->GiveAbilityWithoutDuplication(Ability, AbilitySpecHandle,
                                                                  static_cast<int32>(AbilityCDO->GetAbilityInputID()));
        }
    }
}
...
void ARaidBossCharacterBase::GiveAndActivateAbilities() const
{
    ...
    for (const auto Ability : DefaultActivateAbilities)
    {
        if (Ability)
        {
            ...
            AbilitySystemComponent->GiveAbilityWithoutDuplication(Ability, OutAbilitySpecHandle,
                                                                  static_cast<int32>(AbilityCDO->GetAbilityInputID()));
...
void ARaidBossCharacterBase::GiveSkillAbilities()
{
    ...
    for (const auto PlayerSkill : PlayerSkills)
    {
        if (PlayerSkill)
        {
            ...
            AbilitySystemComponent->GiveAbilityWithoutDuplication(PlayerSkill, AbilitySpecHandle,
                                                                  static_cast<int32>(AbilityCDO->GetAbilityInputID()));
        }
    }
}
