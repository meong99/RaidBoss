...
void AWeapon::ApplyWeaponStatToOwner()
{
    ...
    if (AbilitySystemComponent)
    {
        ...
        FGameplayEffectSpecHandle OutGoingSpec = AbilitySystemComponent->MakeOutgoingSpec(
            WeaponData.AdditiveEffectToCharacter, 1, ContextHandle);

        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Character_Stat_AttackPower,
                                                   WeaponData.AttackPower);
        OutGoingSpec.Data->SetSetByCallerMagnitude(RaidBossGameplayTags::Get().Character_Stat_AttackRange,
