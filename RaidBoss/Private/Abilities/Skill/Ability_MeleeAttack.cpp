                                           ...
                                           const FGameplayEventData* TriggerEventData)
{
    ...
    if (CurrentWeapon->GetWeaponData().WeaponAnimations.UseAnims.IsValidIndex(CurrentCombo))
    {
        ...
        PlayMontageAndWait = UAbilityTask_PlayMontageAndWait::CreatePlayMontageAndWaitProxy(
            this, FName(GetName()), CurrentWeapon->GetWeaponData().WeaponAnimations.UseAnims[CurrentCombo],
            1 * AttackSpeed);
    }
    ...
    UAbilityTask_WaitGameplayEvent* WaitCanActivateNextAttackEvent =
        UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, RaidBossGameplayTags::Get().Animation_Notify_CanActivateNextAttack,
                                                          OwnerCharacter, true);
    UAbilityTask_WaitGameplayEvent* WaitAttackPointEvent =
        UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, RaidBossGameplayTags::Get().Animation_Notify_AttackPoint,
                                                          OwnerCharacter, true);
    UAbilityTask_WaitGameplayEvent* WaitResetComboEvent =
        UAbilityTask_WaitGameplayEvent::WaitGameplayEvent(this, RaidBossGameplayTags::Get().Animation_Notify_ComboReset,
                                                          OwnerCharacter, true);
...
void UAbility_MeleeAttack::ApplyEffectToTarget(const TArray<AActor*>& TargetActors)
{
    ...
    for (const auto& TargetActor : TargetActors)
    {
        ...
        FGameplayAbilityTargetDataHandle    TargetData = UAbilitySystemBlueprintLibrary::AbilityTargetDataFromActor(
            TargetActor);

        TArray<FActiveGameplayEffectHandle> EffectHandles =
            ApplyGameplayEffectToTarget(CurrentSpecHandle, CurrentActorInfo, CurrentActivationInfo, TargetData,
                                        EffectClass, 1);
